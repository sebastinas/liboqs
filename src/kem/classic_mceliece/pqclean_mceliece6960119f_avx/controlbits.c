...
void PQCLEAN_MCELIECE6960119F_AVX_sort_63b(int n, uint64_t *x) {
    if (n <= 1) {
        return;
    }
    PQCLEAN_MCELIECE6960119F_AVX_sort_63b(n / 2, x);
    PQCLEAN_MCELIECE6960119F_AVX_sort_63b(n / 2, x + n / 2);
    merge_63b(n / 2, x, 1);
}
...
void PQCLEAN_MCELIECE6960119F_AVX_controlbits(unsigned char *out, const uint32_t *pi) {
    unsigned int i;
    unsigned char *c;
